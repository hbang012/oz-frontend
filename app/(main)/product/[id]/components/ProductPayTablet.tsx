...
import EstimateTablet from '@/app/(main)/product/[id]/components/EstimateTablet';
import QuantitySelector from '@/app/(main)/product/[id]/components/QuantitySelector';
...
export default function ProductPayTablet({}: ProductPayProps) {
  // 데이터
  const { id } = useParams();
  ...
  return (
    <div className="flex flex-col" style={{ padding: '0px 30px' }}>
      {/* 다른 옵션 추가 버튼 */}
      ...
      {/* 옵션 견적값 */}
      <EstimateTablet pricing={pricing} />
    </div>
  );
}
